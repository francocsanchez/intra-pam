...
- Los accesos del navegador hacia perfil central no deben usar navegacion interceptada por `next/link`; deben disparar una navegacion real del browser.
- Si la sesion central no se invalida al salir, priorizar `POST` directo del navegador a `/logout` de Auth Central mediante un formulario autoenviado desde la ruta local `/logout`, en lugar de depender de redirects entre aplicaciones.
